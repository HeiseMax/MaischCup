...
def calculate_fraction_of_wins(Score, player_id):
    total_matches = Score.query.filter((Score.player1_id == player_id)).count()
    if total_matches == 0:
        return 0  # Avoid division by zero
    won_matches = Score.query.filter_by(player1_id=player_id, win=True).count()
    return round(won_matches / total_matches, 3)
...
def calculate_total_points_received(db, Score, player_id):
    ...
    return points_received_as_player1
...
def calculate_ranking(db, Player, Score):
    ...
    for player in players:
        player_data = {
            ...
            "win_fraction": calculate_fraction_of_wins(Score, player.id),
    ...
    # ranking.sort(key=lambda x: (x["win_fraction"], x["point_difference"]), reverse=True)
